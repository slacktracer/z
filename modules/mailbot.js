...
(function (
    ...
    function mailbot(filePath, fileName, usuario) {
        fs.readFile(filePath, function onRead(error, fileData) {
            if (error) {
                throw error;
            }
            let mail = {
            ...
            transporter.sendMail(mail, function onSent(error, info) {
                if (error) {
                    modules.logger.error(`Erro no envio de e-mail. Detalhes: ${error}`);
                    throw error;
                }
